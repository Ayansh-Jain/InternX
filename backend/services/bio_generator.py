...
def generate_bio_with_llm(relevant_chunks: List[str], job_role: str, job_description: str) -> str:
    ...
    # Check for keys in order: Groq (default for this feature), Gemini (already in .env), then OpenAI
    groq_key = os.environ.get("GROQ_API_KEY")
    gemini_key = os.environ.get("GEMINI_API_KEY")
    openai_key = os.environ.get("OPENAI_API_KEY")
    
    if not any([groq_key, gemini_key, openai_key]):
        raise ValueError("No AI API key found. Please set GROQ_API_KEY, GEMINI_API_KEY, or OPENAI_API_KEY in your .env file.")
...
"""
    ...
    if groq_key:
        ...
        # Adding system instruction to the prompt since flash 1.5 prefers it there or in SysInstruction
        full_prompt = f"System: You are a professional resume writer and recruiter assistant. You must obey all formatting constraints perfectly without adding your own commentary.\n\nUser: {prompt}"
